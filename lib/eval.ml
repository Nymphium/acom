...
let bind_args xs vs =
  ...
  @@
  if xl > vl
  then Array.(make (xl - vl) Null |> to_list) @ vs
  else if xl < vl
  then Base.List.take vs xl
  else vs
;;
...
let%test _ =
  let stmts =
    Nlist.from_list
      [ Expression
          (Call
             ( Call (Value (Builtin SetTimeout), [ Value (Num 5000) ])
             , [ Value
                   (Fun
                      ( []
                      , Nlist.from_list
                          [ Expression
                              (Call (Value (Builtin ConsoleLog), [ Value (Num 100) ]))
                          ] ))
               ] ))
      ]
  in
